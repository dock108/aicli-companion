...
export class ActivityMonitor extends EventEmitter {
  ...
  checkSuspiciousActivity(activity) {
    ...
    const recentCommands = recentActivities.filter(a => 
      a.type === 'command' && 
    ...
    if (recentCommands.length >= this.alertThresholds.rapidCommands.count) {
      ...
      const commandsInWindow = recentCommands.filter(c => 
        new Date(c.timestamp).getTime() > windowStart
      );
    ...
    if (activity.type === 'command' && activity.command && activity.command.includes('rm')) {
      const recentDeletions = recentActivities.filter(a => 
        a.type === 'command' && 
        a.command && 
    ...
    if (activity.type === 'file_operation' && activity.size > this.alertThresholds.largeFileOps.size) {
      const largeFil

Ops = recentActivities.filter(a => 
        a.type === 'file_operation' && 
        a.size > this.alertThresholds.largeFileOps.size
      );
  ...
  calculateTrends() {
    ...
    const lastHour = this.activities.filter(a => 
      new Date(a.timestamp).getTime() > hourAgo
    );
    
    const lastDay = this.activities.filter(a => 
  ...
  generateReport(options = {}) {
    ...
    let activities = sessionId 
      ? (this.sessionActivities.get(sessionId) || [])
      : this.activities;
