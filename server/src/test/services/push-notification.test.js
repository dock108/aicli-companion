...
import { PushNotificationService, pushNotificationService } from '../../services/push-notification/index.js';
...
const mockStoreMessage = mock.fn();

test('PushNotificationService', async (t) => {
  // Set up mocks before each test group
  const originalAPNsClient = PushNotificationService.prototype.apnsClient;
  const originalMessageFormatter = PushNotificationService.prototype.messageFormatter;
  const originalNotificationTypes = PushNotificationService.prototype.notificationTypes;
  ...
  await t.test('sendPushNotification', async (tt) => {
    await tt.test('should send generic push notification', async () => {
      ...
      assert.strictEqual(result.success, true);
      assert.ok(consoleSpy.mock.calls.some(call => 
    ...
    await tt.test('should handle not configured state', async () => {
      ...
      assert.ok(consoleSpy.mock.calls[0].arguments[0].includes('Push notifications not configured'));
  ...
  await t.test('sendStallAlert', async (tt) => {
    await tt.test('should send stall alert', async () => {
      ...
      assert.strictEqual(result.success, true);
      assert.ok(consoleSpy.mock.calls.some(call => 
        call.arguments[0].includes('Stall alert sent')
      ));
  ...
  await t.test('sendErrorNotification', async (tt) => {
    await tt.test('should send error notification', async () => {
      ...
      assert.ok(consoleSpy.mock.calls.some(call => 
        call.arguments[0].includes('Error notification sent')
      ));
  ...
  await t.test('shutdown', async (tt) => {
    await tt.test('should call APNs client shutdown', () => {
      ...
      assert.ok(consoleSpy.mock.calls[0].arguments[0].includes('Push notification service shut down'));
