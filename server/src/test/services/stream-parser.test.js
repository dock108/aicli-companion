...
test('ClaudeStreamParser', async (t) => {
  await t.test('Basic parsing', async (tt) => {
    await tt.test('should parse simple text', () => {
    ...
    await tt.test('should handle multi-line text', () => {
      const parser = new ClaudeStreamParser();
    ...
    await tt.test('should split on empty lines', () => {
      const parser = new ClaudeStreamParser();
  ...
  await t.test('Code block handling', async (tt) => {
    await tt.test('should detect code blocks with language', () => {
      const parser = new ClaudeStreamParser();
    ...
    await tt.test('should handle code blocks without language', () => {
      const parser = new ClaudeStreamParser();
    ...
    await tt.test('should handle incomplete code blocks', () => {
      const parser = new ClaudeStreamParser();
      const chunks = parser.parseData('```javascript\nconst x = 1;', false);
  ...
  await t.test('Buffer management', async (tt) => {
    await tt.test('should accumulate data in buffer', () => {
      const parser = new ClaudeStreamParser();
    ...
    await tt.test('should clear buffer after final chunk', () => {
      const parser = new ClaudeStreamParser();
  ...
  await t.test('Mixed content handling', async (tt) => {
    await tt.test('should handle text followed by code', () => {
      const parser = new ClaudeStreamParser();
    ...
    await tt.test('should handle code followed by text', () => {
      const parser = new ClaudeStreamParser();
      const content = '```js\ncode\n```\n\nMore text here';
      const chunks = parser.parseData(content, true);
  ...
  await t.test('Incremental parsing', async (tt) => {
    await tt.test('should handle incremental code block parsing', () => {
      const parser = new ClaudeStreamParser();
  ...
  await t.test('Edge cases', async (tt) => {
    await tt.test('should handle empty input', () => {
      const parser = new ClaudeStreamParser();
      const chunks = parser.parseData('', true);
    ...
    await tt.test('should handle only whitespace', () => {
      const parser = new ClaudeStreamParser();
      const chunks = parser.parseData('   \n  \n   ', true);
      assert.strictEqual(chunks.length, 1);
    ...
    await tt.test('should handle multiple empty lines', () => {
      const parser = new ClaudeStreamParser();
  ...
  await t.test('Reset functionality', async (tt) => {
    await tt.test('should reset parser state', () => {
      const parser = new ClaudeStreamParser();
