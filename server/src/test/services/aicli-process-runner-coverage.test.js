...
describe('AICLIProcessRunner - Comprehensive Coverage', () => {
  ...
  beforeEach(() => {
    ...
    mockSpawn = mock.fn((command, _args, _options) => {
  ...
  describe('processOutput', () => {
    it('should handle empty output', () => {
      ...
      assert.strictEqual(
        reject.mock.calls[0].arguments[0].message,
        'AICLI CLI returned empty output'
      );
    });
    ...
    it('should handle output with no valid JSON objects', () => {
      ...
      assert.strictEqual(
        reject.mock.calls[0].arguments[0].message,
        'No valid JSON objects found in AICLI CLI output'
      );
    ...
    it('should handle generic JSON parse errors', () => {
      ...
      assert.strictEqual(reject.mock.calls.length, 1);
      assert.ok(
        reject.mock.calls[0].arguments[0].message.includes('Failed to parse AICLI CLI response')
      );
  ...
  describe('findAICLICommand', () => {
    ...
    it('should try different command names in non-test environment', () => {
      ...
      processRunner.spawnFunction = mock.fn((cmd, _args, _options) => {
        if (cmd === 'claude') {
    ...
    it('should return default claude if no commands found', () => {
      ...
      processRunner.spawnFunction = mock.fn((_cmd, _args, _options) => {
        throw new Error('Command not found');
      });
  ...
  describe('Interactive Session (createInteractiveSession)', () => {
    it('should handle interactive session initialization with session ID', async () => {
      ...
      setImmediate(() => {
        mockProcess.stdout.emit(
          'data',
          Buffer.from('{"type":"system","subtype":"init","session_id":"existing-session"}\n')
        );
      });
    ...
    it('should handle stdout chunks without valid JSON', async () => {
      ...
      setImmediate(() => {
        ...
        mockProcess.stdout.emit(
          'data',
          Buffer.from('{"type":"system","subtype":"init","session_id":"test-123"}\n')
        );
      });
  ...
  describe('sendToInteractiveSession', () => {
    it('should send message and handle response', async () => {
      ...
      setImmediate(() => {
        ...
        mockProcess.stdout.emit(
          'data',
          Buffer.from('{"type":"result","result":"Response","session_id":"test-session"}\n')
        );
      });
  ...
  describe('Stream Handling', () => {
    it('should handle stream chunks properly', async () => {
      ...
      const executePromise = processRunner.executeAICLICommand(
        { sessionId: 'test-session', workingDirectory: '/test/dir' },
        'test'
      );
      ...
      setImmediate(() => {
        mockProcess.stdout.emit(
          'data',
          Buffer.from('{"type":"stream","content":"chunk1"}\n{"type":"result","result":"done"}\n')
        );
  ...
  describe('Error Recovery', () => {
    ...
    it('should handle timeout scenarios', async () => {
      ...
      setImmediate(() => {
        mockProcess.stdout.emit(
          'data',
          Buffer.from('{"type":"system","subtype":"init","session_id":"test"}\n')
        );
      });
