...
describe('UAT: Performance & Load Testing', () => {
        ...
        const req = http.get(`${baseUrl}/health`, (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              assert.ok(responseTime < 100, `Health check took ${responseTime}ms, should be < 100ms`);
    ...
    it('should establish WebSocket connections within 200ms', async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.on('open', () => {
          ...
          assert.ok(connectionTime < 200, `WebSocket connection took ${connectionTime}ms, should be < 200ms`);
  ...
  describe('Concurrent Connection Handling', () => {
    it('should handle 20 concurrent HTTP requests efficiently', async () => {
      ...
      assert.ok(avgResponseTime < 200, `Average response time ${avgResponseTime}ms should be < 200ms`);
    ...
    it('should handle 10 concurrent WebSocket connections', async () => {
      ...
      try {
        ...
        console.log(`Concurrent WebSocket test: ${numConnections} connections in ${totalConnectionTime}ms`);
        console.log(`Average connection time: ${(connectionTimes.reduce((a, b) => a + b, 0) / connectionTimes.length).toFixed(2)}ms`);
        
        // All connections should be established quickly
        assert.ok(totalConnectionTime < 3000, `Total connection time ${totalConnectionTime}ms should be < 3000ms`);
        ...
        assert.ok(totalPingTime < 1000, `Total ping-pong time ${totalPingTime}ms should be < 1000ms`);
        ...
        ws.on('message', (data) => {
          try {
            ...
            if (message.type === 'error' && message.data.error.includes('Unknown message type')) {
              ...
              assert.ok(processingTime < 1000, `Large message processing took ${processingTime}ms, should be < 1000ms`);
              resolve();
            }
          } catch (error) {
            clearTimeout(timeout);
          ...
          try {
            ...
            if (message.type === 'pong') {
              ...
              if (responseCount === numMessages) {
                ...
                assert.ok(totalTime < 5000, `Message burst took ${totalTime}ms, should be < 5000ms`);
                resolve();
              }
            }
          } catch (error) {
  ...
  describe('Memory Usage Monitoring', () => {
    it('should not leak memory with connection cycles', async () => {
      ...
      assert.ok(memoryIncrease < 10, `Memory increase ${memoryIncrease.toFixed(2)}MB should be < 10MB`);
    });
  });
