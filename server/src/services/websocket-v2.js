...
export function setupWebSocket(wss, claudeService, authToken) {
  ...
  wss.on('connection', (ws, request) => {
    ...
    getClaudeCodeVersion().then(claudeCodeVersion => {
...
async function handlePermissionMessage(clientId, requestId, data, claudeService, clients) {
  const { sessionId, response, remember } = data;
  ...
  try {
    ...
    sendErrorMessage(clientId, requestId, 'PERMISSION_ERROR', error.message, clients, { sessionId });
  }
}
